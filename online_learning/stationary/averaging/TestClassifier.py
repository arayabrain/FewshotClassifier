...
def mkData(N): # This is now making N batches of size BS
    ...
    for i in range(N):
        batch = []
        for j in range(BS):
            data[i*BS+j,DIM] = tlabels[i*BS+j]
            batch.append(data[i*BS+j])
    ...
    for i in range(N):
        ...
        lbatch = []
        for j in range(BS):
            batch.append(data[N*BS+i*BS+j])
            lbatch.append(tlabels[N*BS+i*BS+j])
...
embedding = lasagne.layers.NonlinearityLayer(lasagne.layers.GlobalPoolLayer(nin_final), nonlinearity = lambda x: 0.1*x)

# Hidden-to-hidden
emb_input = lasagne.layers.InputLayer((None,256))
l_htoh = lasagne.layers.NonlinearityLayer(emb_input, nonlinearity = lambda x: 0.9*x)

observer_network = lasagne.layers.CustomRecurrentLayer(tr_input, embedding, l_htoh, nonlinearity = None)

emb_reshape = lasagne.layers.ReshapeLayer(observer_network, (-1,seqlen,256,1))
...
out, messages = lasagne.layers.get_output([output,observer_network])
...
def testModel(epoch):
	...
	for i in range(24):
		...
		err = test(data,tdata,tlabels)
		losses.append(np.mean(err,axis=(0,2)))
	
	losses = np.array(losses)
	outs,msg = process(data,tdata)
	np.savetxt("losscurve.txt",np.mean(losses,axis=0))
	np.savetxt("message.txt",msg[0])
